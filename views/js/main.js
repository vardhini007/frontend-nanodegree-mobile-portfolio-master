...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
  	// Put all the pizzas in one array
  	// Moved the calculation of the dx and newwidth variables outside the loop, as all the pizzas are the same size, thus do not need
    // to calculate each iteration.
    // Changed querySelectorAll to getElementsByClassName
    // By these changes, time to resize pizzas is less than 5ms.
...
// Moved the pizzaDivs variable outside the loop so that  DOM does not get queried each iteration.
